...
;(function($){
    ...
    var defaults = {
        ...
        eHidden : undefined,
        /**
         * Menu display without search
         * @type function
         * @param data {array[Object]} selected items data
         */
        displayWithoutSearch : false//渲染数据的时候是否搜索数据
    };
    ...
    SelectMenu.prototype.populate = function() {
        ...
        if (p.displayWithoutSearch){
            self.displayResultsWithOutSearch(self)
        }
        else self.suggest(self);
    ...
    SelectMenu.prototype.displayResultsWithOutSearch = function (self) {
        var data = self.option.data;
        // var p = self.option;
        var p = self.option, el = self.elem, css = self.css_class;
        if(data.length){
            var arr_candidate = data;
            for (var i = 0; i < arr_candidate.length; i++) {
                // console.log("*****displayResults******json.candidate********")
                var itemText = '', custom = false, row = arr_candidate[i];
                if(p.formatItem && $.isFunction(p.formatItem)){
                    try {
                        itemText = p.formatItem(row);
                        custom = true;
                ...
                var icon = $('<div>').html('<i class="iconfont icon-selected">').addClass(css.selected_icon),
                    ...
                    li = $('<li>').append(icon).append(text).attr('pkey' , arr_candidate[i].id);
        ...
        if(p.displayFun && $.isFunction(p.displayFun)){
            p.displayFun();
        }
    }
    /**
    ...
    SelectMenu.prototype.calcResultsSize = function(self) {
        ...
        var setListHeight = function(){
            ...
            // console.log("***************setListHeight结束***************");
        };
        var scrollFlag = hasScroll();
        // console.log(scrollFlag);
        var rePosition = function(){
            // console.log("***************rePosition开始***************");
